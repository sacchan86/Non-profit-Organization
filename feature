Feature: Adding New Contact
  As an Organization leader
  So I can search and organize the member lists quick and easy
  I want to store a new contact
  
  
Feature: Search for members info
  As a Group leader
  So I can search for my own group member information
  I want to contact specific member
  
  
Feature: Check up the practice schedule
  As a Group member
  So I can check my practice / performance time and dates
  I want to share the performance date
  
  
Feature: Interested in some performances
  As a Guests
  So I can look up when is the performance for specific group
  I want to add the schedule into my calendar
